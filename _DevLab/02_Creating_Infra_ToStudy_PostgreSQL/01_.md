...
## Question06:
can you now create the Ansible playbook with jinja2 that run the creating and configuration of the needed lab components on these 3 Nodes?

## Answer06:
This is a sophisticated request. We are going to build an **Automated High-Availability Cluster** using **Patroni**.

Why **Patroni**? Because manual streaming replication is fragile. Patroni (used by companies like Zalando and Gitlab) automatically manages the Failover. If `PG1` dies, Patroni talks to `ETCD` (on the Infra node), promotes `PG2`, and tells `HAProxy` to reroute traffic.

### The Ansible Project Structure

On your **Infra-Node**, create this folder structure:
...
### 1. `inventory.ini`
**Action:** Update the IPs to match what your VMs actually got from your Router.

```ini
[infra]
192.168.1.100 hostname=infra

[db_primary]
192.168.1.101 hostname=pg1

[db_replica]
192.168.1.102 hostname=pg2
...
[all:vars]
ansible_user=your_linux_username
ansible_ssh_pass=your_linux_password
ansible_become_pass=your_linux_password
# The subnet allowed to connect to Postgres
allowed_subnet=192.168.1.0/24
```

---

### 2. `site.yml` (The Playbook Entry Point)
...
Sets up the `/etc/hosts` file so nodes can talk by name.
...
- name: Install Basic Tools
  apt:
    name: [curl, git, sudo, gnupg2, lsb-release, vim]
    state: present
...
### 4. Role: `infra` (ETCD + HAProxy)
**File:** `roles/infra/tasks/main.yml`

```yaml
---
- name: Install HAProxy and ETCD
  apt:
    name: [haproxy, etcd-server, etcd-client]
    state: present
...
**File:** `roles/infra/templates/haproxy.cfg.j2`
This is the magic. HAProxy checks port `8008` (Patroni API) to see who is the Leader.
...
# Dashboard (Optional, access via http://infra-ip:7000)
listen stats
    mode http
    bind *:7000
    stats enable
...
listen postgres_write
    ...
    server pg1 192.168.1.101:5432 maxconn 100 check port 8008
    server pg2 192.168.1.102:5432 maxconn 100 check port 8008

# READ-ONLY POOL (Replica)
listen postgres_read
    bind *:5001
    option httpchk
    http-check expect status 503 # Patroni returns 503 for Replicas
...
### 5. Role: `postgres` (PG16 + AGE + Patroni)
...
- name: Update Apt after Repo Add
  apt:
    update_cache: yes

- name: Install Postgres, Extensions, and Patroni
  apt:
    name:
      - postgresql-16
...
- name: Stop default Postgres (Patroni will manage it)
  service:
    name: postgresql
    state: stopped
    enabled: no
...
# --- BUILD APACHE AGE ---
- name: Clone Apache AGE
  git:
    repo: 'https://github.com/apache/age.git'
    dest: /tmp/age
    version: master # Ideally verify this supports PG16 or find specific tag

- name: Build and Install AGE
  shell: |
    cd /tmp/age
    make PG_CONFIG=/usr/lib/postgresql/16/bin/pg_config install
  args:
    creates: /usr/lib/postgresql/16/lib/age.so
...
**File:** `roles/postgres/templates/patroni.yml.j2`
This configuration auto-bootstraps the cluster.

```jinja2
scope: uncodeway_cluster
namespace: /db/
...
1.  **SSH** into your `Infra-Node` VM.
2.  Install Ansible: `sudo apt install ansible`.
3.  Create the files above.
4.  **Key Exchange (Critical):** Ansible needs to SSH into PG1/PG2 without password prompt failures.
    ```bash
...
### How to Connect from Visual Studio (Windows)

Once the playbook finishes:
*   **Host:** `192.168.1.100` (The Infra Node IP)
*   **Port:** `5000` (This is HAProxy!)
*   **Database:** `postgres`
*   **User:** `postgres`
*   **Password:** `admin_password`

HAProxy will automatically forward you to whichever node is currently the Primary. If you shut down PG1, HAProxy will briefly pause, then forward you to PG2 once Patroni promotes it.
